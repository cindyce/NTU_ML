...
class multi_fcn_autoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder1 = nn.Sequential(
            nn.Linear(64 * 64 * 3, 128*10),
            nn.ReLU(),
            nn.Linear(128*10, 64*10),
            nn.ReLU(), 
            nn.Linear(64*10, 12*30), 
            nn.ReLU(), 
            nn.Linear(12*30, 3*80)
        )
        
        self.encoder2 = nn.Sequential(
            nn.Linear(64 * 64 * 3, 128*20),
            nn.ReLU(),
            nn.Linear(128*20, 64*15),
            nn.ReLU(),
            nn.Linear(64*15, 12*25), 
            nn.ReLU(), 
            nn.Linear(12*25, 3*80)
        )
        
        self.encoder3 = nn.Sequential(
            nn.Linear(64 * 64 * 3, 128*5),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            nn.Linear(3*80, 12*30),
            nn.ReLU(), 
            nn.Linear(12*30, 64*15),
            nn.ReLU(),
            nn.Linear(64*15, 128*20),
            nn.ReLU(), 
            nn.Linear(128*20, 64 * 64 * 3), 
            nn.Tanh()
        ) 
...
model = GlobalContextBlock(inplanes=3, ratio=0.25).cuda()
...
image=img[0,:].view(3,64,64).permute(1, 2, 0).cpu().numpy()
output_img=output[0,:].view(3,64,64).permute(1, 2, 0).cpu().numpy()
plt.figure()
plt.imshow(image)
plt.figure()
