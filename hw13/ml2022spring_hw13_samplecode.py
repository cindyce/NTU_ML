...
cfg = {
    ...
    'weight_decay': 1e-5,
    'grad_norm_max': 10,
    'n_epochs': 2000, # train more steps to pass the medium baseline.
    'patience': 300,
}
...
def loss_fn_kd(student_logits, labels, teacher_logits, alpha=0.5, temperature=1.0):
    ...
    # If you have no idea, please take a look at the provided useful link above.
    loss = alpha*temperature**2*kl_loss(F.log_softmax(student_logits/temperature), F.log_softmax(teacher_logits/temperature)) + (1-alpha)*ce_loss(student_logits, labels)
    return loss
    # pass
