        ...
        for e, epoch in enumerate(range(self.config["n_epoch"])):
            ...
            for i, data in enumerate(progress_bar):
                ...
                for name, param in self.D.named_parameters():
                    ...
                    if name =='l1.0.weight':
                        param_norm1 = param.grad.data.norm(2)
                        total_norm1 += param_norm1.item()
                    if name =='l1.2.0.weight' or name =='l1.2.1.weight':
                        param_norm2 = param.grad.data.norm(2)
                        total_norm2 += param_norm2.item()
                    if name =='l1.3.0.weight' or name =='l1.3.1.weight':
                        param_norm3 = param.grad.data.norm(2)
                        total_norm3 += param_norm3.item()
                    if name =='l1.4.0.weight' or name =='l1.4.1.weight':
                        param_norm4 = param.grad.data.norm(2)
                        total_norm4 += param_norm4.item()
                    if name =='l1.5.weight':
                        param_norm5 = param.grad.data.norm(2)
                        total_norm5 += param_norm5.item()
                        #total_norm += param_norm.item() ** 2
                        #total_norm = total_norm ** (1. / 2)
                        #print(np.log10(param_norm.item()))
                    #print('-->name:', name, '-->grad_requirs:',param.requires_grad, \
                		 #' -->grad_value:',param.grad)
                
                '''
                gnorm = nn.utils.clip_grad_norm_(self.D.parameters(), config["clip_value"]) # grad norm clipping prevents gradient exploding
                print(gnorm)'''
            ...
            gn = np.log([total_norm1,total_norm2,total_norm3,total_norm4,total_norm5])
            '''#gn = np.log([param_norm1.item(),param_norm2.item(),param_norm3.item(),param_norm4.item(),param_norm5.item()])
            layer_num = np.arange(1, len(gn)+1)
            plt.figure()
            plt.plot(layer_num, gn)
            #plt.title("") # title
            plt.xlabel("Discriminator layer") # x label
            plt.ylabel("Gradient norm (log scale)") # y label
            plt.legend(['Weight clipping (c=%1.2f)' %self.config["clip_value"]])'''
...
if __name__ == '__main__':    
    ...
    trainer.inference(f'{workspace_dir}/checkpoints/2022-04-12_19-08-57_GAN/G_199.pth') # you have to modify the path when running this line
