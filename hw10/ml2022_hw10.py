...
def DIM(image, prob=0.6):
    rnd_h = random.randint(int(image.size(2)*0.8), image.size(2))
    rnd_w = random.randint(int(image.size(3)*0.8), image.size(3))
    h = abs(image.size(2) - rnd_h)
    w = abs(image.size(3) - rnd_w)
    pad_left = random.randint(0, w)
    pad_top = random.randint(0, h)
    pad_right = w - pad_left
    pad_bottom = h - pad_top
    tf = transforms.Compose([
        transforms.Resize((rnd_h, rnd_w)),
        transforms.Pad((pad_left, pad_top, pad_right, pad_bottom), fill=0, padding_mode="constant"), 
        ])
    return tf(image) if torch.rand(1)<prob else image
...
def ifgsm(model, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=20):
    ...
    for i in range(num_iter):
        ...
        x_adv.requires_grad = True # need to obtain gradient of x_adv, thus set required grad
        loss = loss_fn(model(DIM(x_adv)), y) # calculate loss
        loss.backward() # calculate gradient
        # fgsm: use gradient ascent on x_adv to maximize loss
        grad = x_adv.grad.detach()
...
def mifgsm(model, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=20, decay=1.0):
    ...
    for i in range(num_iter):
        ...
        # grad = .....
        grad = x_adv.grad.detach() 
        grad = grad / torch.norm(torch.abs(grad), p=1, dim=(1,2,3), keepdim=True)
...
class ensembleNet(nn.Module):
    ...
    def forward(self, x):
        ensemble_logits = torch.zeros(x.size()[0], 10).to(device)
        for i, m in enumerate(self.models):
            # TODO: sum up logits from multiple models  
            logit = self.softmax(m(x))
            ensemble_logits = ensemble_logits + logit
...
create_dir(root, 'mifgsm_ensemble', adv_examples, adv_names)
