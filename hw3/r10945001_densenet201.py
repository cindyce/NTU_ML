...
# Model
from torchvision.models import resnet50,densenet121,densenet169,densenet201,densenet161,resnext50_32x4d
# Augmentation
...
myseed = 1234567  # set a random seed for reproducibility
...
train_tfm = transforms.Compose([
    ...
    #transforms.RandomAffine(degrees=(-90,90), translate=(0.1, 0.1), scale=(0.9, 0.9)),
    transforms.Resize((128, 128)),
...
train_data, valid_data = train_valid_split(dataset, 0.25, myseed)

train_set = FoodDataset(train_path, train_data, tfm=train_tfm)
valid_set = FoodDataset(valid_path, valid_data, tfm=test_tfm)'''

# Construct datasets.
# The argument "loader" tells how torchvision reads the data.
train_set1 = FoodDataset(os.path.join(_dataset_dir,"training"), tfm=train_tfm)
train_set2 = FoodDataset(os.path.join(_dataset_dir,"training"), tfm=test_tfm)
...
model = densenet201(pretrained=False, num_classes=11).to(device)
...
for epoch in range(n_epochs):
    ...
    if valid_acc > best_acc:
        with open(f"./{_exp_name}_log.txt","w"):
...
model_best = densenet201(pretrained=False, num_classes=11).to(device)
