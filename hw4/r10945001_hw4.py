...
Original file is located at
    https://colab.research.google.com/drive/1Z5Jhm2FFN5f7YZ6JFghlrnpc8fg97kQf
...
class SelfAttentionPooling(nn.Module):
	...
	def forward(self, out):
		# C = softmax(Wc*H^T)H
		# Wc is a trainable parameter
		softmax = nn.functional.softmax
		att_w = softmax(self.W(out).squeeze(-1)).unsqueeze(-1) # att_w: (batch size, length, 1)
		utter = torch.sum(out*att_w, dim = 1)
...
class Classifier(nn.Module):
	def __init__(self, d_model=360, n_spks=600, dropout=0.1):
		super().__init__()
		...
		self.SAP = SelfAttentionPooling(input_dim=d_model)
	...
	def forward(self, mels, labels):
		...
		# mean pooling => stats: (batch size, d_model)
		#stats = out.mean(dim=1)
		# self-attention pooling => stats: (batch size, d_model)
		stats = self.SAP(out)
        
		# out: (batch, n_spks)
		out = self.pred_layer(stats)
        
		loss, out = self.loss(out, labels)
...
def main(
	...
	for feat_paths, mels in tqdm(dataloader):
		with torch.no_grad():
			mels = mels.to(device)
			labels = torch.zeros([len(mels)]).to(device) # fake labels
			outs, _ = model(mels, labels.long())
